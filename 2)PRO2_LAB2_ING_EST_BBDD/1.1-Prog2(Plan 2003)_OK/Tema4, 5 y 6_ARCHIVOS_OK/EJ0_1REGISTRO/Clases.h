...
class Persona{
    protected:
        ...
        bool estado;
    public:
        Persona(int d=0, const char *n="S/N", const char *a="S/A", Fecha f=Fecha(), const char *t="S/T", bool estado = false);
...
Persona::Persona(int d, const char *n, const char *a, Fecha f, const char *t, bool estado)
: dni(d), fechaNacimiento(f), estado(estado){
    strcpy(nombre,n);
    strcpy(apellido,a);
    strcpy(telefono,t);
}
...
void cargarCadena(char *pal, int tam){
    cin.ignore(); ///limpia el buffer de entrada para que la carga se haga sin caracteres que hayan quedado sin usar
    int i;
    for(i=0; i<tam;i++){
        pal[i]=cin.get();
        if(pal[i]=='\n')break;
