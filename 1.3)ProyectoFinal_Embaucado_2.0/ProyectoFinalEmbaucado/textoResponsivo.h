...
void dibujarTextoCentrado(string texto, int fila = 2){
...
void mostrarMensaje(string texto, int fila = -1, int columna = -1) {
    int lineas = altoMaxFilas(texto) + 1; // el +1 es por que si escribo 1 sola no habra enter
    ...
    dibujarRectangulo(posXCentrado - 2, posYCentrado - 1, columnasMax + 4, lineas + 2 /*una linea arriba y otra abajo*/, ' ');

    // Dibuja el texto centrado
    int posInicial = 0;
    int posFinal = 0;
    for (int i = 1; i <= lineas; i++) { // por cada linea busco donde esta el \n
