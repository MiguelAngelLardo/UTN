...
namespace Dao
{
  public class AccesoDatos
  {
    ...
    public AccesoDatos(string ruta = "Data Source= localhost; Initial Catalog = BDSucursales; User ID = sa; Password=Miguel-1234;Encrypt=False")
    {
      _ruta = ruta;
    }
    ...
    public DataTable daoGetTablaAdap(string consultaSelect) //no uso try catch para que la excepcion la maneje quien llama al metodo
    {
   ...
   SqlCommand cmd = new SqlCommand();
     ...
     */
    public int ejecutarSP(SqlCommand cmd)
    {
      using (SqlConnection cn = new SqlConnection(_ruta))
      {
        ...
        try
        {
          ...
          return cmd.ExecuteNonQuery();//devuelve int filas afectadas
        }
        catch (Exception ex)
        {
          return -1;
