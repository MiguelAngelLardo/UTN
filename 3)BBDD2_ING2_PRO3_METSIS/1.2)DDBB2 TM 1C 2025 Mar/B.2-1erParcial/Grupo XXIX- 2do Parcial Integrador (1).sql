...
BEGIN
    ...
    IF EXISTS (SELECT cod_Art FROM inserted JOIN ARTICULOS ON ARTICULOS.cod_Art = inserted.codArt_DV
...
BEGIN
    ...
    UPDATE ARTICULOS
    SET stock_Art = stock_Art + inserted.cantidad_PD
    FROM ARTICULOS A
    INNER JOIN inserted  ON A.cod_Art = inserted.codigoArticulo_PD
    INNER JOIN deleted  ON inserted.codigoPedido_PD = deleted.codigoPedido_PD
    WHERE deleted.Cumplido_PD = 0 AND inserted.Cumplido_PD = 1;

 
    INSERT INTO AUDITORIAS (codigoArticulo_AU, stockAnterior_AU, stockNuevo_AU)
    SELECT 
        inserted.codigoArticulo_PD,
        ARTICULOS.stock_Art - inserted.cantidad_PD,  
        ARTICULOS.stock_Art                  
    FROM ARTICULOS 
    INNER JOIN inserted  ON ARTICULOS.cod_Art = inserted.codigoArticulo_PD
    INNER JOIN deleted  ON inserted.codigoPedido_PD = deleted.codigoPedido_PD
    WHERE deleted.Cumplido_PD = 0 AND inserted.Cumplido_PD = 1;
END;
GO
