...
- DNI del dueño
- Saldo
- Estado (1 - Activa, 0 - Dada de baja)

Viajes.dat
- Número de viaje
- Número de tarjeta
- Medio de transporte (1 - Colectivo, 2 - Subte, 3 - Tren)
- Fecha del viaje
- Importe del viaje
- Estado (1 - Activa, 0 - Dada de baja)

ll) Generar un archivo para registrar por cada una de las tarjetas el importe total gastado por viajes el
año actual. Cada registro debe tener el número de tarjeta, el DNI del dueño y el importe por los viajes.
///
m) Dar de baja lógica a las tarjetas que tengan un saldo negativo menor a -15000.

n) Dar de baja a todos los viajes del año 2020.
o) Generar un archivo con todos los registros de viaje del año 2020, y luego dar de baja física en el
archivo de viajes a los registros viajes de ese año.
p) Sumar al saldo de las tarjetas con viajes en el presente año $1000.
q) Sumar al saldo de las tarjetas el importe del IVA del mes de agosto. El porcentaje de IVA es del 21%.

2) Una peluquería tiene un archivo (color.dat) con la información de las coloraciones (tinturas) realizadas a
los/las clientes en lo que va del año. Cada registro contiene los siguientes campos:
Código de tintura (número entero entre el 100 y el 135), fecha (día, mes y año), género (‘H’: hombre; ‘M’:
mujer; ‘O’: otro), tipo de pelo.
Hacer un programa para informar el/los códigos de tintura que hayan registrado al menos 10 coloraciones
en el mes de marzo.
3) Parques Nacionales tiene, entre otros, los siguientes archivos en su sistema:
Animales.dat: Código de animal (int), nombre (char[30]), clasificación (char[5]), zona de habitat (entero
entre 1 y 10), y estado (1= registro activo; 0: registro inactivado o borrado)
Avistajes.dat: Código de animal (int), fecha (día, mes y año), zona de avistaje (entero entre 1 y 10) El archivo
tiene registros con los animales vistos desde el año 2000 a la fecha.
Se pide hacer una función para generar un archivo nuevo (peligro.dat) con aquellos animales que no se
hayan avistado desde enero de 2016. Nota: se pueden desarrollar todas las funciones que se consideren
necesarias.
3) La Cámara de Transporte necesita realizar un análisis del rendimiento de los autobuses utilizados por las
empresas de colectivos durante el año pasado. En su sistema dispone de un conjunto de archivos, entre los
que se destacan:
Empresa.dat
Código de empresa (char[5]), nombre, dirección, tipo de empresa (1: local.; 2: interurbano; 3: nacional; 4:
internacional)
Viajes.dat

Número de viaje, código de empresa, marca del autobús (1 a 10), cantidad de km del viaje (float), cantidad
de combustible consumido (float), fecha.
A partir de los datos contenidos en los archivos se pide resolver los siguientes puntos:
...
//Ver como se hace baja fisica con maxi.
...
//ver si esta bien el resultado con el excel con Maxi
