...
Gasto::Gasto(){ // : Transaccion() -> es otra forma de llamar al constructor, _establecimiento(""), _idMetodoPago(0)
...
Gasto::Gasto(int idTransaccion, const std::string& descripcion, const Fecha& fecha, int idCategoria, int idMetodoPago, const std::string& establecimiento, float monto, bool estado) {
	Transaccion::Transaccion(idTransaccion, descripcion, fecha, idCategoria, monto, estado);
